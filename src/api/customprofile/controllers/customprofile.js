...
module.exports = {
  ...
  async fetchMyProfile(ctx) {
    try {
      ...
      const filteredProfile = await this.filterProfile(profile, userId);
  ...
  async fetchUserProfileById(ctx) {
    try {
      const { id } = ctx.params;
  ...
  async filterProfile(profile, userId) {
    ...
    return {
      ...
      isRequestReceiver: relation?.destinataire?.id === userId,
      isMyProfile: profile.id === userId, // Ensure this property is added
