...
module.exports = createCoreController("api::parcour.parcour", ({ strapi }) => ({
  async create(ctx) {
    try {
      ...
      const createdPathway = await strapi
        .service("api::parcour.parcour")
        .createPathway(data, userId);
  ...
  async find(ctx) {
    try {
      ...
      const result = await strapi
        .service("api::parcour.parcour")
        .findPathways(_page, _limit, _q, userId);
  ...
  async update(ctx) {
    try {
      ...
      const updatedPathway = await strapi
        .service("api::parcour.parcour")
  ...
  async delete(ctx) {
    try {
      ...
      await strapi.service("api::parcour.parcour").deletePathway(id);
...
//     try {
//       const { id } = ctx.params;
//       const { nom, type, etablissement, autoApprentissage } = ctx.request.body;

//       console.log("Nom:", nom);
//       console.log("Type:", type);
//       console.log("Établissement:", etablissement);
//       console.log("Auto-apprentissage:", autoApprentissage);
...
//             nom: nom,
//             type: type,
//             etablissement: etablissement === null ? "" : etablissement,
//             autoApprentissage:
//               autoApprentissage === null ? false : autoApprentissage,
//           },
//         }
//       );
//       ctx.send({
